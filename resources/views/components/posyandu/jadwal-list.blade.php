...
<div class="flex flex-col h-full relative">
    <div class="pb-3 flex items-center justify-between shrink-0">
        ...
        <span class="bg-blue-100 text-blue-700 text-[10px] font-extrabold px-3 py-1 rounded-full">{{ count($jadwals) }} Jadwal</span>
    </div>
        ...
        <div class="flex flex-col gap-3">
            @forelse($jadwals as $jadwal)
                <div class="bg-white rounded-[1.25rem] p-4 shadow-sm border border-slate-100 hover:shadow-sm border border-slate-200/60 hover:border-blue-100/50 transition-all group relative overflow-hidden flex flex-col gap-3">
                    ...
                    <div>
                        <h4 class="text-[14px] font-extrabold text-slate-800">{{ $jadwal['judul'] }}</h4>
                        <div class="flex flex-wrap items-center gap-2 mt-2">
                            <div class="flex items-center gap-1.5 text-xs text-blue-700 font-bold bg-blue-50/50 px-2 py-1 rounded-md border border-blue-100/50">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-3.5 h-3.5">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                                {{ \Carbon\Carbon::parse($jadwal['tanggal'])->translatedFormat('d M Y') }}
                            </div>
                            <div class="flex items-center gap-1.5 text-xs text-indigo-700 font-bold bg-indigo-50/50 px-2 py-1 rounded-md border border-indigo-100/50">
                    ...
                    <div class="flex items-center gap-1.5 text-xs font-medium text-slate-500 bg-slate-50 px-2 py-1.5 rounded-md border border-slate-100">
            ...
            @empty
                <div class="flex flex-col items-center justify-center p-8 bg-white rounded-[1.25rem] border border-slate-100 h-full min-h-[260px] text-center gap-3">
                    <div class="w-16 h-16 bg-slate-50 flex items-center justify-center rounded-2xl mb-2">
                    ...
                    <div>
                        <span class="block text-[13px] font-extrabold text-slate-800">Belum ada agenda terjadwal</span>
                        <span class="block text-[11px] font-medium text-slate-400 mt-1.5 max-w-[200px] mx-auto leading-relaxed">Jadwal kegiatan posyandu akan muncul di sini.</span>
                    </div>
                </div>
            @endforelse
        </div>
