...
<div class="w-full min-h-screen bg-[#F8FAFC] pb-24 lg:pb-16 text-slate-800 antialiased font-sans">
    <div class="max-w-7xl mx-auto px-3.5 sm:px-6 lg:px-8 py-3.5 sm:py-6 flex flex-col gap-3.5 sm:gap-5">
        
        {{-- ── 1. WELCOME COMMAND HEADER (Fully Fluid & Responsive) ── --}}
        <div class="bg-white border-t-4 border-t-teal-700 border-x border-b border-slate-200/90 rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-2xs flex flex-col md:flex-row md:items-center justify-between gap-3.5 sm:gap-6">
            
            {{-- Left: Sapaan & Live Status --}}
            <div class="flex-1 min-w-0">
                <div class="flex items-center gap-1.5 sm:gap-2 mb-1.5 sm:mb-2 text-xs font-semibold text-slate-500">
                    <span class="inline-flex items-center gap-1 sm:gap-1.5 px-2 sm:px-2.5 py-0.5 rounded-full bg-teal-50 text-teal-900 border border-teal-200/80 font-bold text-[10.5px] sm:text-[11px] shadow-2xs">
                        <span class="w-1.5 h-1.5 rounded-full bg-teal-600 animate-pulse"></span>
                        <span class="truncate max-w-[140px] sm:max-w-none">{{ $activityLocation ?? ($posyanduName ?? 'Posyandu') }}</span>
                    </span>
                    <span class="text-slate-300">•</span>
                    <span class="inline-flex items-center gap-1 text-slate-600 font-medium text-[11px] sm:text-xs">
                ...
                <div class="flex flex-wrap items-center gap-1.5 sm:gap-2">
                    <h1 class="text-lg sm:text-2xl lg:text-[26px] font-extrabold text-slate-900 tracking-tight leading-snug">
                        Selamat bertugas, <span class="text-teal-800">{{ $cleanName }}</span>
                    </h1>
                    @if($roleText)
                        <span class="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] sm:text-[11px] font-bold bg-slate-100 text-slate-600 border border-slate-200 shadow-2xs">{{ $roleText }}</span>
                    @endif
                </div>
                
                {{-- Live Operational Pulse (Symmetrical 2-Column on Mobile) --}}
                <div class="grid grid-cols-2 gap-2 mt-2.5 sm:flex sm:flex-wrap sm:items-center sm:gap-2">
                    <div class="inline-flex items-center justify-center sm:justify-start gap-1.5 px-2.5 py-1 rounded-lg bg-emerald-50 text-emerald-900 border border-emerald-200 text-[11px] sm:text-xs font-semibold shadow-2xs min-w-0">
                        <x-icon name="check-circle" weight="bold" class="text-emerald-600 text-xs sm:text-sm shrink-0" />
                        <span class="truncate">Selesai: <strong class="font-bold text-emerald-950">{{ $sudah }}/{{ $total }}</strong> ({{ $percent }}%)</span>
                    </div>
                    @if($belum > 0)
                    <div class="inline-flex items-center justify-center sm:justify-start gap-1.5 px-2.5 py-1 rounded-lg bg-amber-50 text-amber-950 border border-amber-200 text-[11px] sm:text-xs font-semibold shadow-2xs min-w-0">
                        <x-icon name="clock" weight="bold" class="text-amber-600 text-xs sm:text-sm shrink-0" />
                        <span class="truncate">Antrean: <strong class="font-bold text-amber-950">{{ $belum }} balita</strong></span>
                    </div>
                    @endif
                </div>
            </div>
            ...
            {{-- Right: Actions --}}
            <div class="grid grid-cols-2 gap-2 sm:flex sm:items-center sm:gap-2.5 shrink-0 pt-2.5 sm:pt-0 border-t border-slate-100 sm:border-0">
                <a href="{{ route('balita.create') }}" 
                   class="flex items-center justify-center gap-1.5 px-3 py-2.5 bg-white hover:bg-slate-50 active:scale-[0.98] border border-slate-300 text-slate-800 rounded-xl text-xs sm:text-sm font-bold shadow-2xs transition-all">
                    <x-icon name="user-plus" weight="bold" class="text-teal-700 text-sm sm:text-base shrink-0" />
                    <span class="truncate">+ Balita Baru</span>
                </a>
                <a href="{{ route('balita.index') }}" 
                   class="flex items-center justify-center gap-1.5 px-3.5 py-2.5 bg-teal-700 hover:bg-teal-800 active:scale-[0.98] text-white rounded-xl text-xs sm:text-sm font-bold shadow-sm shadow-teal-700/30 hover:shadow transition-all">
                    <x-icon name="scales" weight="bold" class="text-white text-sm sm:text-base shrink-0" />
                    <span class="truncate">Mulai Timbang</span>
                </a>
            </div>
        </div>
        ...
        <div class="bg-amber-50/90 border border-amber-200/90 rounded-2xl p-3.5 sm:p-4.5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-2xs">
            <div class="flex items-start sm:items-center gap-3 min-w-0">
                <div class="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-amber-500 text-white flex items-center justify-center shrink-0 shadow-xs ring-4 ring-amber-100 text-base sm:text-xl">
