...
<div class="flex flex-col w-full bg-slate-50/50 min-h-screen">
    ...
    <div class="bg-white px-5 pt-5 pb-4 shadow-sm border-b border-slate-100 sticky top-0 z-20">
        <div class="max-w-2xl mx-auto w-full flex items-center gap-3">
            ...
            @else
                <span class="flex-shrink-0 bg-teal-50 text-teal-600 border border-teal-200 text-[11px] font-bold px-2.5 py-1 rounded-full">
                    Mendatang
                </span>
            @endif
        </div>
    ...
    <div class="max-w-2xl mx-auto w-full px-5 py-5 pb-28 flex flex-col gap-4">
        ...
        <div class="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm flex flex-col gap-4">
            ...
            <div class="flex flex-col gap-3">
                <div class="flex items-center gap-3">
                    <div class="w-9 h-9 rounded-xl bg-teal-50 flex items-center justify-center text-teal-600 flex-shrink-0 border border-teal-100">
        ...
        <div class="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm">
            <div class="flex items-center justify-between mb-3">
                ...
                <span class="text-sm font-black text-teal-600">{{ $sudahDiukur }}/{{ $totalBalita }}</span>
            </div>
            <!-- Progress Bar -->
            ...
            <div class="flex gap-3">
                <div class="flex-1 bg-emerald-50 border border-emerald-100 rounded-xl p-3 flex flex-col items-center">
                    <span class="text-xl font-black text-emerald-700">{{ $sudahDiukur }}</span>
                    <span class="text-[10px] font-bold text-emerald-600 uppercase tracking-wide mt-0.5">Selesai</span>
                </div>
            ...
            <div class="flex flex-col divide-y divide-slate-50">
                @forelse($balitaList as $balita)
                    <a href="{{ route('balita.show', $balita['id'] ?? '') }}" class="flex items-center justify-between px-5 py-3.5 hover:bg-slate-50 transition-colors group">
                        <div class="flex items-center gap-3">
                            <!-- Avatar -->
                            <div class="w-9 h-9 rounded-full bg-slate-100 flex items-center justify-center text-slate-400 flex-shrink-0 group-hover:bg-teal-50 group-hover:text-teal-500 transition-colors border border-slate-200/60">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5">
                                    <path fill-rule="evenodd" d="M7.5 6a4.5 4.5 0 119 0 4.5 4.5 0 01-9 0zM3.751 20.105a8.25 8.25 0 0116.498 0 .75.75 0 01-.437.695A18.683 18.683 0 0112 22.5c-2.786 0-5.433-.608-7.812-1.7a.75.75 0 01-.437-.695z" clip-rule="evenodd" />
                                </svg>
                            </div>
                            <span class="text-sm font-semibold text-slate-800 group-hover:text-teal-700 transition-colors">{{ $balita['name'] }}</span>
                        </div>
                        <!-- Status -->
                        @if($balita['statusType'] === 'success')
                            <div class="flex items-center gap-1.5 bg-emerald-50 text-emerald-700 px-2 py-1 rounded-lg border border-emerald-100 flex-shrink-0">
    ...
    <div class="fixed bottom-20 lg:bottom-0 inset-x-0 z-20 pointer-events-none">
        <div class="max-w-2xl mx-auto w-full px-5 py-3 pointer-events-auto">
            <a href="{{ route('balita.index') }}"
               class="w-full flex items-center justify-center gap-2 bg-teal-600 hover:bg-teal-700 text-white px-5 py-4 rounded-2xl font-bold text-base shadow-lg shadow-teal-500/20 transition-all duration-200 focus:outline-none focus:ring-4 focus:ring-teal-500/30">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-5 h-5">
