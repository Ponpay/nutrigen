...
<div class="w-full min-h-screen bg-[#F4F7F6] pb-28 lg:pb-16 text-slate-800 antialiased">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-5 sm:pt-7 flex flex-col gap-5 sm:gap-6">
        
        {{-- ── 1. EXECUTIVE HERO BANNER (Warna Identik NutriGen Teal, Glass Telemetry, Proporsional) ── --}}
        <div class="relative bg-gradient-to-br from-teal-800 via-teal-700 to-emerald-800 rounded-3xl p-5 sm:p-7 lg:p-8 text-white shadow-[0_12px_36px_-8px_rgba(13,148,136,0.35)] border border-teal-600/30 overflow-hidden">
            
            {{-- Ambient Light Sheen --}}
            <div class="absolute -right-10 -top-10 w-60 h-60 bg-emerald-400/20 rounded-full blur-3xl pointer-events-none"></div>
            <div class="absolute left-1/3 -bottom-12 w-52 h-52 bg-teal-400/15 rounded-full blur-2xl pointer-events-none"></div>
            ...
            <div class="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-5 sm:gap-6">
                ...
                <div class="flex-1 max-w-xl">
                    ...
                    <p class="text-xs sm:text-sm text-teal-100/90 font-normal mt-1.5 leading-relaxed">
                        Pantau pertumbuhan balita secara presisi, deteksi stunting sejak dini, dan sinkronkan data penimbangan ke Puskesmas.
                    </p>
                </div>

                {{-- Right: Integrated Glassmorphic Telemetry Strip (Sleek, Modern, High-Taste) --}}
                <div class="bg-teal-950/40 backdrop-blur-md border border-white/15 rounded-2xl p-3 sm:p-4 shadow-inner grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 shrink-0">
                    
                    {{-- 1. Total Balita --}}
                    <div class="bg-white/10 hover:bg-white/15 transition-colors border border-white/10 rounded-xl px-3.5 py-2.5 flex flex-col justify-center min-w-[100px]">
                        <span class="text-[10px] font-bold uppercase tracking-wider text-teal-200/80">Total Balita</span>
                        <div class="flex items-baseline gap-1 mt-0.5">
                            <span class="text-xl sm:text-2xl font-black text-white leading-none">{{ $total }}</span>
                            <span class="text-[10px] text-teal-200/60 font-medium">anak</span>
                        </div>
                    </div>

                    {{-- 2. Selesai Ukur --}}
                    <div class="bg-emerald-500/20 hover:bg-emerald-500/25 transition-colors border border-emerald-400/30 rounded-xl px-3.5 py-2.5 flex flex-col justify-center min-w-[100px]">
                        <span class="text-[10px] font-bold uppercase tracking-wider text-emerald-200 flex items-center gap-1">
                            <span class="w-1.5 h-1.5 rounded-full bg-emerald-400"></span> Selesai
                        </span>
                        <div class="flex items-baseline gap-1 mt-0.5">
                            <span class="text-xl sm:text-2xl font-black text-emerald-300 leading-none">{{ $sudah }}</span>
                            <span class="text-[10px] text-emerald-200/70 font-bold">({{ $percent }}%)</span>
                        </div>
                    </div>

                    {{-- 3. Belum Hadir --}}
                    <div class="bg-amber-500/20 hover:bg-amber-500/25 transition-colors border border-amber-400/30 rounded-xl px-3.5 py-2.5 flex flex-col justify-center min-w-[100px]">
                        <span class="text-[10px] font-bold uppercase tracking-wider text-amber-200 flex items-center gap-1">
                            <span class="w-1.5 h-1.5 rounded-full bg-amber-400"></span> Belum
                        </span>
                        <div class="flex items-baseline gap-1 mt-0.5">
                            <span class="text-xl sm:text-2xl font-black text-amber-300 leading-none">{{ $belum }}</span>
                            <span class="text-[10px] text-amber-200/70 font-medium">anak</span>
                        </div>
                    </div>

                    {{-- 4. Perlu Revisi --}}
                    <div class="bg-rose-500/20 hover:bg-rose-500/25 transition-colors border border-rose-400/30 rounded-xl px-3.5 py-2.5 flex flex-col justify-center min-w-[100px]">
                        <span class="text-[10px] font-bold uppercase tracking-wider text-rose-200 flex items-center gap-1">
                            <span class="w-1.5 h-1.5 rounded-full bg-rose-400 animate-pulse"></span> Revisi
                        </span>
                        <div class="flex items-baseline gap-1 mt-0.5">
                            <span class="text-xl sm:text-2xl font-black text-rose-300 leading-none">{{ $statRevisi ?? 0 }}</span>
                            <span class="text-[10px] text-rose-200/70 font-medium">data</span>
                        </div>
                    </div>
        ...
        <div class="bg-rose-50/90 border border-rose-200/90 rounded-2xl p-4 sm:p-4.5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-2xs">
                ...
                <div>
                    ...
                    @else
                        <div class="py-6 text-center flex flex-col items-center justify-center text-slate-400 mt-1">
                            ...
                            <span class="text-[11px] text-slate-400 mt-0.5 mb-2.5">Buat jadwal kegiatan posyandu.</span>
