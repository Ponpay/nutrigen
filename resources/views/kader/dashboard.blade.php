...
<div class="w-full min-h-screen bg-[#F8FAFC] pb-24 lg:pb-16 text-slate-800 antialiased font-sans">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 sm:py-6 flex flex-col gap-4 sm:gap-5">
        
        {{-- ── 1. UNIFIED APP HEADER (Clean, Professional & Balanced) ── --}}
        <div class="bg-white border border-slate-200/90 rounded-2xl p-5 sm:p-6 shadow-2xs flex flex-col md:flex-row md:items-center justify-between gap-4 sm:gap-6">
            ...
            <div class="flex-1 min-w-0">
                ...
                <h1 class="text-xl sm:text-2xl lg:text-[26px] font-bold text-slate-900 tracking-tight leading-snug">
                    Selamat bertugas, <span class="text-teal-700">{{ $kaderName ?? 'Ibu Kader' }}</span>
                </h1>
                
                <p class="text-xs sm:text-[13px] text-slate-500 font-medium mt-1 flex flex-wrap items-center gap-x-3 gap-y-1">
                    <span>Selesai diukur: <strong class="text-slate-800 font-semibold">{{ $sudah }}/{{ $total }} balita</strong> ({{ $percent }}%)</span>
                    @if($belum > 0)
                        <span class="text-slate-300">•</span>
                        <span>Sisa antrean: <strong class="text-amber-700 font-semibold">{{ $belum }} balita</strong></span>
                    @endif
                </p>
            </div>
            ...
            <div class="grid grid-cols-2 gap-2 sm:flex sm:items-center sm:gap-2.5 shrink-0 pt-2 sm:pt-0 border-t border-slate-100 sm:border-0">
                <a href="{{ route('balita.create') }}" 
                   class="flex items-center justify-center gap-1.5 px-4 py-2.5 bg-white hover:bg-slate-50 active:scale-[0.98] border border-slate-200 text-slate-700 text-xs sm:text-sm font-semibold rounded-xl shadow-2xs transition-all">
                    <x-icon name="user-plus" weight="bold" class="text-teal-700 text-sm" />
                    <span>+ Balita Baru</span>
                </a>
                <a href="{{ route('balita.index') }}" 
                   class="flex items-center justify-center gap-1.5 px-4.5 py-2.5 bg-teal-600 hover:bg-teal-700 active:scale-[0.98] text-white text-xs sm:text-sm font-semibold rounded-xl shadow-xs transition-all">
                    <x-icon name="scales" weight="bold" class="text-white text-sm" />
                    <span>Mulai Penimbangan</span>
                </a>
            </div>
        </div>

        {{-- ── 2. ALERT PERLU REVISI (Restrained & High Contrast) ── --}}
        @if(isset($statRevisi) && $statRevisi > 0)
        <div class="bg-amber-50/90 border border-amber-200/90 rounded-xl p-3.5 sm:p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-2xs">
            <div class="flex items-start sm:items-center gap-3 min-w-0">
                <div class="w-8 h-8 rounded-lg bg-amber-500 text-white flex items-center justify-center shrink-0 shadow-2xs text-base">
                ...
                <div class="flex flex-col min-w-0">
                    ...
                    <p class="text-xs text-amber-900/90 font-medium mt-0.5 leading-relaxed">
            ...
            <a href="{{ route('balita.index', ['filter' => 'ditolak']) }}" 
               class="w-full sm:w-auto shrink-0 h-8.5 px-3.5 bg-amber-700 hover:bg-amber-800 text-white rounded-lg text-xs font-semibold shadow-2xs transition-all flex items-center justify-center gap-1 cursor-pointer">
        ...
        {{-- ── 3. FOUR METRIC CARDS (Unified Surface & Proportional Sizing) ── --}}
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            ...
            <div class="bg-white border border-slate-200/90 hover:border-slate-300 rounded-2xl p-4 sm:p-5 flex flex-col justify-between shadow-2xs hover:shadow-xs transition-all">
                <div class="flex items-center justify-between">
                    ...
                    <div class="w-8 h-8 rounded-lg bg-slate-50 text-slate-600 flex items-center justify-center border border-slate-100 text-sm">
            ...
            <div class="bg-white border border-slate-200/90 hover:border-slate-300 rounded-2xl p-4 sm:p-5 flex flex-col justify-between shadow-2xs hover:shadow-xs transition-all">
                <div class="flex items-center justify-between">
                    ...
                    <div class="w-8 h-8 rounded-lg bg-emerald-50 text-emerald-700 flex items-center justify-center border border-emerald-100 text-sm">
            ...
            <div class="bg-white border border-slate-200/90 hover:border-slate-300 rounded-2xl p-4 sm:p-5 flex flex-col justify-between shadow-2xs hover:shadow-xs transition-all">
                <div class="flex items-center justify-between">
                    ...
                    <div class="w-8 h-8 rounded-lg bg-amber-50 text-amber-700 flex items-center justify-center border border-amber-100 text-sm">
            ...
            <div class="bg-white border border-slate-200/90 hover:border-slate-300 rounded-2xl p-4 sm:p-5 flex flex-col justify-between shadow-2xs hover:shadow-xs transition-all">
                <div class="flex items-center justify-between">
                    ...
                    <div class="w-8 h-8 rounded-lg bg-rose-50 text-rose-700 flex items-center justify-center border border-rose-100 text-sm">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-5">
            ...
            <div class="lg:col-span-7 bg-white border border-slate-200/90 rounded-2xl shadow-2xs overflow-hidden flex flex-col">
                ...
                {{-- Child List (Distinct Structured Card Tiles) --}}
