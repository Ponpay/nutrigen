...
<div class="w-full min-h-screen bg-[#F8FAFC] pb-24 lg:pb-16 text-slate-800 antialiased font-sans">
    <div class="max-w-7xl mx-auto px-3.5 sm:px-6 lg:px-8 py-4 sm:py-6 flex flex-col gap-4 sm:gap-5">
        ...
        {{-- ── 1. WELCOME COMMAND HEADER (Soft Tint, High Contrast & Live Measurement Status) ── --}}
        <div class="bg-gradient-to-r from-teal-50/90 via-emerald-50/40 to-slate-50 border border-teal-200/80 rounded-2xl sm:rounded-3xl p-4.5 sm:p-6 shadow-2xs flex flex-col md:flex-row md:items-center justify-between gap-4 sm:gap-6">
            
            {{-- Left: Sapaan & Live Status --}}
            <div class="flex-1 min-w-0">
                <div class="flex flex-wrap items-center gap-1.5 sm:gap-2 mb-2">
                    <span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 sm:px-3 sm:py-1 bg-teal-100/90 border border-teal-300/80 text-teal-950 rounded-full text-[11px] sm:text-xs font-bold uppercase tracking-wider shadow-2xs">
                    ...
                    <span class="inline-flex items-center gap-1.5 text-[11px] sm:text-xs text-slate-700 font-semibold bg-white/95 px-2.5 py-0.5 sm:py-1 rounded-full border border-slate-200 shadow-2xs">
                ...
                <div class="mt-2.5 flex flex-wrap items-center gap-x-4 gap-y-1.5 text-xs text-slate-600 font-medium">
                    <div class="flex items-center gap-1.5">
                        ...
                        <span>Selesai diukur: <strong class="text-slate-900 font-bold">{{ $sudah }}/{{ $total }} balita</strong> ({{ $percent }}%)</span>
                    </div>
                    @if($belum > 0)
                    <div class="flex items-center gap-1.5">
                        <span class="w-2 h-2 rounded-full bg-amber-500"></span>
                        <span>Sisa antrean: <strong class="text-amber-800 font-bold">{{ $belum }} balita</strong></span>
                    </div>
                    @endif
                </div>
            </div>
            ...
            {{-- Right: Actions --}}
            <div class="grid grid-cols-2 gap-2 sm:flex sm:items-center sm:gap-2.5 shrink-0 pt-2 sm:pt-0 border-t border-teal-100/70 sm:border-0">
                <a href="{{ route('balita.create') }}" 
                   class="flex items-center justify-center gap-1.5 sm:gap-2 px-3.5 sm:px-4 py-2.5 sm:py-3 bg-white hover:bg-slate-50 active:scale-[0.98] border border-slate-300 text-slate-800 rounded-xl text-xs sm:text-sm font-bold shadow-2xs transition-all">
                    <x-icon name="user-plus" weight="bold" class="text-teal-700 text-sm sm:text-base" />
                    <span>+ Balita Baru</span>
                </a>
                <a href="{{ route('balita.index') }}" 
                   class="flex items-center justify-center gap-1.5 sm:gap-2 px-4.5 sm:px-5 py-2.5 sm:py-3 bg-teal-700 hover:bg-teal-800 active:scale-[0.98] text-white rounded-xl text-xs sm:text-sm font-bold shadow-sm shadow-teal-700/30 hover:shadow transition-all">
